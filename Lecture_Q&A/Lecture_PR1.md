Peer comments:
1. **Idea is creative**: The idea is creative because the player does not platform on real objects, but on the shadows projected by them.
...
5. **Limit the light control**: The light control should be limited at first, for example to a few fixed angles, so the game is easier to build, test, and balance.