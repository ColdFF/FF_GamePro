...
public class LevelEndBlackoutController : MonoBehaviour
{
    ...
    [Header("Final Storyline")]
    public bool showFinalStorylineWhenNoNextLevel = false;
    public string[] finalStorylineLines =
    {
        "You have completed every level.",
        "Every shadow became a path when you changed your perspective.",
        "Thank you for playing ShadowPath."
    };
    public float finalStorylineTextFadeDuration = 0.8f;
    public float finalStorylineLineHoldDuration = 2.5f;
    public float finalStorylineBetweenLineDelay = 0.18f;

    private float[] originalLightIntensities;
    private Canvas blackoutCanvas;
    private GameObject successMenuObject;
    private TextMeshProUGUI finalStorylineText;
    ...
    IEnumerator PlayBlackoutRoutine()
    {
        ...
        if (showSuccessMenuAfterBlackout && ShouldPlayFinalStoryline())
        {
            yield return PlayFinalStoryline();
        }
    ...
    /// <summary>
    /// Purpose: Decides whether the final completion message should play before the success menu.
    /// Input: Final-story settings and next-level configuration.
    /// Output: True only for an ending with no next level configured.
    /// </summary>
    bool ShouldPlayFinalStoryline()
    {
        return showFinalStorylineWhenNoNextLevel &&
            string.IsNullOrWhiteSpace(nextLevelSceneName) &&
            finalStorylineLines != null &&
            finalStorylineLines.Length > 0;
    }

    /// <summary>
    /// Purpose: Plays the final completion story text on the black screen.
    /// Input: Final-story text and timing settings.
    /// Output: Each line fades in and out before the success menu appears.
    /// </summary>
    IEnumerator PlayFinalStoryline()
    {
        EnsureFinalStorylineText();

        if (finalStorylineText == null)
        {
            yield break;
        }

        finalStorylineText.gameObject.SetActive(true);
        finalStorylineText.alpha = 0f;
        finalStorylineText.text = string.Empty;

        for (int i = 0; i < finalStorylineLines.Length; i++)
        {
            string line = finalStorylineLines[i];
            if (string.IsNullOrWhiteSpace(line))
            {
                continue;
            }

            finalStorylineText.text = line;
            yield return FadeFinalStorylineText(0f, 1f, finalStorylineTextFadeDuration);
            yield return WaitUnscaled(finalStorylineLineHoldDuration);
            yield return FadeFinalStorylineText(1f, 0f, finalStorylineTextFadeDuration);
            finalStorylineText.text = string.Empty;
            yield return WaitUnscaled(finalStorylineBetweenLineDelay);
        }

        finalStorylineText.gameObject.SetActive(false);
    }

    /// <summary>
    /// Purpose: Creates the final-story text object on the blackout canvas.
    /// Input: Existing blackout canvas or CanvasGroup.
    /// Output: Centered TextMeshPro text matching chapter-transition style.
    /// </summary>
    void EnsureFinalStorylineText()
    {
        if (finalStorylineText != null)
        {
            return;
        ...
        float storylineScale = GetFinalStorylineScale();

        GameObject textObject = new GameObject("FinalStorylineMessage");
        textObject.transform.SetParent(blackoutCanvas.transform, false);

        RectTransform rectTransform = textObject.AddComponent<RectTransform>();
        rectTransform.anchorMin = new Vector2(0.5f, 0.5f);
        rectTransform.anchorMax = new Vector2(0.5f, 0.5f);
        rectTransform.pivot = new Vector2(0.5f, 0.5f);
        rectTransform.sizeDelta = new Vector2(1500f, 480f) * storylineScale;
        rectTransform.anchoredPosition = Vector2.zero;

        finalStorylineText = textObject.AddComponent<TextMeshProUGUI>();
        finalStorylineText.alignment = TextAlignmentOptions.Center;
        finalStorylineText.enableWordWrapping = true;
        finalStorylineText.fontSize = 88f * storylineScale;
        finalStorylineText.color = new Color(0.92f, 0.86f, 0.72f, 1f);
        finalStorylineText.raycastTarget = false;
        finalStorylineText.text = string.Empty;
        finalStorylineText.alpha = 0f;
        finalStorylineText.gameObject.SetActive(false);
    }
    ...
    /// <summary>
    /// Purpose: Matches chapter-transition text scale without resizing the success menu.
    /// Input: Current Game view size and a 1920x1080 reference resolution.
    /// Output: Scale factor equivalent to CanvasScaler's width/height match at 0.5.
    /// </summary>
    float GetFinalStorylineScale()
    {
        CanvasScaler existingScaler = blackoutCanvas != null
            ? blackoutCanvas.GetComponent<CanvasScaler>()
            : null;
        ...
        float widthRatio = Mathf.Max(1f, Screen.width) / 1920f;
        float heightRatio = Mathf.Max(1f, Screen.height) / 1080f;
    ...
    /// <summary>
    /// Purpose: Fades final-story text without depending on gameplay time scale.
    /// Input: Starting alpha, ending alpha, and fade duration.
    /// Output: Text alpha reaches the requested value.
    /// </summary>
    IEnumerator FadeFinalStorylineText(float fromAlpha, float toAlpha, float duration)
    {
        float elapsed = 0f;
        ...
        while (elapsed < safeDuration)
        {
            ...
            finalStorylineText.alpha = Mathf.Lerp(fromAlpha, toAlpha, t);
            yield return null;
        }

        finalStorylineText.alpha = toAlpha;
    }
    ...
    void OnValidate()
    {
        ...
        finalStorylineTextFadeDuration = Mathf.Max(0.01f, finalStorylineTextFadeDuration);
        finalStorylineLineHoldDuration = Mathf.Max(0f, finalStorylineLineHoldDuration);
        finalStorylineBetweenLineDelay = Mathf.Max(0f, finalStorylineBetweenLineDelay);
    }
}
